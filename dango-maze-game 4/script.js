...
function isCollidingWithBoundary(dango) {
    return (
        dango.x - DANGO_DIAMETER / 2 <= 0 ||
        dango.x + DANGO_DIAMETER / 2 >= canvas.width ||
        dango.y - DANGO_DIAMETER / 2 <= 0 ||
        dango.y + DANGO_DIAMETER / 2 >= canvas.height
    );
}

function isCollidingWithGoal(dango, goal) {
    return (
        dango.x + DANGO_DIAMETER / 2 > goal.x &&
        dango.x - DANGO_DIAMETER / 2 < goal.x + goal.width &&
        dango.y + DANGO_DIAMETER / 2 > goal.y &&
        dango.y - DANGO_DIAMETER / 2 < goal.y + goal.height
    );
}
